...
namespace ATM.ApiServices
{
    public class StaffService
    {
        ...
        public ApiStaffTransactionStatisticsModel GetStaffTransactionStatistics(string accountNumber)
        {
            try
            {
                var loginStr = "";
                var repose = new ApiStaffTransactionStatisticsModel();
                ...
                if (response.IsSuccessStatusCode)
                {
                    loginStr = response.Content.ReadAsStringAsync().Result;
                    JavaScriptSerializer serializer = new JavaScriptSerializer();
                    repose = serializer.Deserialize<ApiStaffTransactionStatisticsModel>(loginStr);
                }
                return repose;
            }
            catch (Exception ex)
